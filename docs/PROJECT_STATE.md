...
- **Added Reversible Report Demo Mode & Polished Frontend:** Added a reversible, local-only `REPORT_DEMO_MODE=true` environment setting that dynamically serves realistic demo data for Dashboard/Progress, Current Affairs, and Tests without polluting the SQLite database or Chroma vector indexes. Implemented the idempotent `python -m src.scripts.seed_report_demo_data` script (with `--clear` support) to easily toggle this mode. Polished the UI across the Dashboard, Current Affairs page, and Tests page to include visible "Report Demo Mode" badges when active. Refactored the Current Affairs error panel to be a non-blocking alert banner that preserves article views. Restored the full set of fallback source adapters in `source_policy.py` and updated domain assertions, ensuring 100% of the 191 backend unit tests and the React build compilation pass cleanly.
- **Completed Current Affairs Sourcing & Fallbacks:** Updated the backend and frontend to support fallback sourcing to other active publishers (like PIB, RBI, MEA, ForumIAS, InsightsIAS, Drishti IAS) when PWOnlyIAS articles are absent. Implemented a temporary `DEMO_MODE` dummy dataset of 10 articles in-memory to populate all tabs (Day-wise, Weekly, Monthly, Subject-wise, and Q&A) for presentations and screenshots, complete with "Demo Data" badges and fallback source indicators. Fixed the Q&A tab rendering defect to display grounded questions and answer/explanation previews properly instead of showing a hardcoded empty state.
