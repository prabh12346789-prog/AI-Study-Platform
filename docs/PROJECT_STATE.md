...
- Completed UPSC Books Feature (PWOnlyIAS Static Books, Chapter Support & Web Reader): Audited and cleaned runtime/demo synthetic records from production database. Added separate UPSC Books discovery (`discover_books`), storage (`BookCollection`, `UPSCBook`, `BookChapter`, `SavedBook`, `BookReadingProgress`), reused `is_valid_pwonlyias_source_url` and `extract_pdf_blocks`, Chroma RAG indexing (`source_type = "upsc_book"`), and learner-facing API (`/upsc-books/subjects`, `/collections`, `/`, `/{id}`, `/{id}/content`, `/{id}/save`, `/saved`, `/{id}/progress`). Added `UPSC Books` navigation item and responsive `UpscBooksPage.tsx` with Subject overview cards, search/filters, Continue Reading, Saved Books, chapter selector, and an internal webpage reader modal with Table of Contents, page references, debounced reading-progress updates, Ask AI / Prelims / Mains / Revision actions, and secondary official PWOnlyIAS PDF/source links.
- Completed UPSC Notes Feature (PWOnlyIAS Static Notes & Web Reader): Added separate UPSC Notes discovery, storage, strict official PWOnlyIAS URL validation (`is_valid_pwonlyias_source_url`), robust `pypdf` block extraction (`extract_pdf_blocks`), checksum deduplication, Chroma RAG indexing, and learner-facing API (`/upsc-notes/subjects`, `/collections`, `/`, `/{id}`, `/{id}/content`, `/{id}/save`, `/saved`, `/{id}/progress`). Added `UPSC Notes` navigation item and responsive `UpscNotesPage.tsx` with Subject overview cards, search/filters, Continue Reading, Saved Notes, and an internal webpage reader modal with Table of Contents, page references, debounced reading-progress updates, Take Quiz action, and secondary official PWOnlyIAS PDF/source links.
...
- Forty-six targeted backend tests across Current Affairs, UPSC Notes, and UPSC Books pass 100%. Backend startup smoke test passes. Frontend typecheck (`npx tsc --noEmit`) and frontend production build (`npm run build`) pass with zero errors.
