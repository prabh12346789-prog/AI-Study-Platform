...
- Populated the active `backend/data/memory.sqlite3` database with 1,992 detailed events covering 926 learning days and 44 calendar months: 1,827.4 study hours, 463 questions, 308 searches, 132 revisions, 84 tests, 48 book opens, and 31 visual roadmaps. Existing Dashboard and Progress Analytics graphs consume these rows through the real activity summary API.
- Every generated row has a stable ID and `details.source=long_term_demo_seed`; a repeat run added zero rows. The script works with Python's standard library so demo data can be restored even while the checked-in virtual environment is broken.
- Dashboard loading repair: normalized the 1,992 seeded SQLite timestamps to the same naive-UTC representation written by SQLAlchemy. Mixed aware/naive values had caused activity summary sorting to raise HTTP 500, which surfaced as `Failed to fetch`. Live `7d`, `30d`, `90d`, `all`, and mentor-dashboard requests now return HTTP 200; future seed runs write the compatible format directly.
- Cross-feature demo depth and Dashboard containment: added six detailed AI Study Coach conversations with paired learner/mentor messages, twelve varied mastery and forgetting-risk topics for Revision Center, and six schema-valid saved Visual Learning roadmaps with local SVG previews. The 308 historical internal searches are now distributed across Chat, Revision, Visual Learning, UPSC Books, and Current Affairs page contexts. All records remain idempotent and tagged as local demo history where applicable.
- Reworked the Dashboard study chart into separate plot and summary rows, increased its contained height, widened the right-side minimum column, allowed long Current Affairs titles to wrap, and switches the main content to a readable stacked layout below 1420px. This prevents chart labels, summary tiles, article titles, and adjacent cards from overlapping at compact desktop widths.

## Desktop UI and Tests error-handling repair — 2026-08-06

- Increased Dashboard desktop typography, card spacing, chart labels, quick actions, and list readability based on the supplied 1118px capture while retaining the existing responsive collapse rules.
- Made the AI Study Coach vertically scrollable on short desktop viewports so the full three-panel workspace and composer remain reachable instead of being clipped below the fold.
- Tests quiz generation now maps unexpected local-model failures to an actionable HTTP 503 response, and the Tests UI identifies a stopped/unreachable FastAPI backend instead of showing the browser's generic `Failed to fetch` message.
- General-subject Fundamental Rights practice now has a reviewed ten-question deterministic fallback when Ollama is unavailable; Ollama remains the primary generator and other arbitrary topics continue to fail transparently rather than inventing questions. Live `POST /tests/prelims/generate` verification returned ten questions from the verified question bank.
- Added an API regression test for unavailable quiz generation. Frontend production build passes. Backend execution is locally blocked because the checked-in `.venv` points to a missing Python 3.12 installation, system Python has no pytest, and the Windows Store Python launcher is access-denied.
- Dashboard information-depth pass adds real-data context throughout: chart totals, daily averages, active/best days, donut-center study time, subject coverage/share, metric definitions, consent scope, richer quick-action descriptions, PDF readiness/page counts, roadmap subjects/node counts, Current Affairs publisher/priority, revision mastery/forgetting risk/reasons, recommendation duration/priority/type, and actionable insight subtitles. Chart and lower-card heights are explicitly contained to prevent desktop overlap. TypeScript and the production build pass.

## Consented long-term learner history — 2026-08-06

- Added `internal_search` to the activity event store for searches made only inside UPSC AI Mentor. Global workspace searches are recorded when a result is selected; Current Affairs and UPSC Books searches are debounced and record one normalized query rather than every keystroke. Page/context metadata is retained, while external browser and device activity remain entirely out of scope.
- Activity summaries now support `30d` and `90d`, return daily activity/study breakdowns, search counts, and the ten most-used search terms. The Dashboard consumes the real 90-day summary and shows focused study, active days, questions, platform searches, subjects explored, and recent search topics with a clear consent explanation.
- Added focused manager/API tests for search persistence, 90-day summaries, and daily history. Live read-only `/activity/summary?period=90d` verification returned the new schema with four historical activity days and 588 recorded study seconds. Frontend production build, Python compilation, and `git diff --check` pass. Pytest execution remains blocked by the broken checked-in virtual-environment interpreter path.
- Multi-year extension adds `1y` and `all` activity periods, first-activity date, lifetime learning-day count, and monthly study/event/search/question rollups. The Dashboard now presents an all-time learning journey plus a stable rolling 12-month bar chart that includes zero-activity months and exposes monthly details on hover. Live `/activity/summary?period=all` returned the first recorded activity on 2026-08-03, four lifetime learning days, and one populated monthly bucket. Backend compilation, frontend production build, and `git diff --check` pass; pytest remains blocked by the existing broken `.venv` interpreter path.
- Analytics usability extension connects a shared 7/30/90-day selector to both the Dashboard study graph and subject-time pie chart. Range changes update study time, questions, subject counts, daily totals/averages, active and best days, subject distribution/share, and adaptive x-axis label spacing. AI Study Coach submissions and successful Visual Learning generation requests now record consented `internal_search` events with appropriate mode, subject, source, and visual-type context. Backend compilation, frontend TypeScript/production build, and `git diff --check` pass.
...
- **Reference-aligned Dashboard redesign (2026-08-05):** Rebuilt the learner Dashboard around the approved dark-navy reference hierarchy with a compact weekly study-goal card, six real navigation actions, seven-day study overview chart, subject-time distribution, Current Affairs snapshot, due-revision list, recent PDFs/roadmaps, mentor recommendations, and study insights. Every visible metric and item is sourced from the existing mentor, activity, Current Affairs, PDF, and roadmap APIs; unavailable sections render professional empty states. Added responsive desktop, tablet, and mobile layouts and a dedicated final-override stylesheet. Frontend production build passes. Screenshot comparison remains blocked because no browser instance is connected to the in-app browser runtime.
- **Phase 2 learning-workspace redesign (2026-08-05):** Redesigned AI Study Coach, Current Affairs, UPSC Books, and Visual Learning against the approved reference layouts without backend or data-store changes. Current Affairs is now one date-based official-source experience (Weekly, Monthly, Subject-wise, and Q&A learner tabs removed) with source/subject/search/saved filters, metadata, compact source health, reader, save, official link, quiz navigation, and AI-draft handoff. UPSC Books now exposes only Prelims and Mains, uses real subject counts and book metadata, preserves the material/PDF reader and progress, and passes section/subject state into Tests; Q/A Bank is excluded. Visual Learning adds source/subject/detail guidance, visual-type cards, staged generation, real SVG preview, grounding, save/download/fullscreen/history, and honest empty/error states. The Coach uses a three-panel desktop workspace with real session context and responsive drawers. Live frontend, health, Current Affairs, Books, visual-roadmap, and conversation endpoints all return HTTP 200; production data currently has no Books, visual roadmaps, or conversations, so their empty states are active. `npx tsc --noEmit` and `npm run build` pass. Browser screenshots remain blocked because no browser instance is connected.
- **Phase 2 screenshot correction and isolated previews (2026-08-05):** Corrected the undersized typography, compressed controls, and excessive dead space shown in the supplied 1158px screenshots by scaling Phase 2 headings, cards, controls, panels, chat messages, subject/book blocks, visual selectors, and responsive breakpoints toward the approved reference. Added a single display-only `phaseTwoFallback.ts` layer for empty API states: selectable example Coach topics, clearly labeled Books subject/catalog previews with persistence-dependent actions disabled, and a clearly labeled Visual Learning timeline workspace with example topics. Real API results always replace previews; a manually selected Current Affairs date with no records still shows the honest empty state. No fallback is written to APIs, SQLite, or local persistence. `npx tsc --noEmit`, `npm run build`, and `git diff --check` pass; browser recapture remains unavailable because no browser instance is connected.
- **Windows Python 3.12 and Current Affairs quiz eligibility verification (2026-08-04):** Verified `backend\.venv\Scripts\python.exe` as Python 3.12.10 and installed the existing pinned `backend\requirements.txt` (all requirements already satisfied). Production SQLite retained 39 active official non-demo Current Affairs records (29 PIB, 10 RBI); the UPSC Books table currently has 0 records. Fixed the shared quiz eligibility rule so concise official releases are accepted when their grounded summary, Prelims relevance, and Mains relevance fields are present, and restored construction of the grounded fact pool while removing the incorrect four-article minimum. Live `/tests/sources` returned `current_affairs.available=true, count=39`; `/current-affairs/qa` returned 39 grounded PIB/RBI items; live quiz generation returned five cited official questions. Current Affairs and Books targeted tests pass 76 with 2 skipped. The automated `backend/tests` suite passes 190 with 6 skipped and one legacy demo-seed failure because production eligibility intentionally excludes demo articles. Root `pytest` collection is additionally blocked when Ollama is offline because `backend/test_ollama.py` executes a connection attempt at import time. Frontend `npx tsc --noEmit` and production build pass. Browser proof remains unavailable because no browser instance was connected to the in-app browser runtime.
- **Invalid Current Affairs quiz recovery (2026-08-04):** Added a shared Current Affairs text sanitizer and strict four-option quiz validation, explicit partial-answer scoring, stored-session ID validation, idempotent submission reuse, and invalid-session recovery. Contaminated unfinished quizzes are returned as invalid, can be idempotently marked `abandoned` with reason `contaminated_source_text`, and are excluded from active-session lookup without deleting quiz rows or completed history. The Tests UI sends one abandon request, shows a recovery loading/error state, clears only Current Affairs quiz state after success, preserves the selected question count, and returns to configuration. Stale PWOnlyIAS-only Current Affairs wording was replaced with verified-material and PIB/RBI/MEA wording. Live API validation abandoned the known contaminated session, returned no active invalid session, preserved the completed-attempt count, and generated one clean five-question quiz with zero validator failures. Directly affected tests pass 32 with 6 skipped; the full backend tests directory passes 201 with 6 skipped and the pre-existing demo-seed incompatibility remains. Backend import, TypeScript, and production build pass. Browser screenshots remain blocked because no browser instance is connected.
...
- **Removed UPSC Notes feature and Fixed Tests feature:** Safely removed the learner-facing UPSC Notes feature from both frontend and backend, and resolved visible defects in the Tests feature. Registered the missing activity events (`test_started`, `prelims_test_completed`, `current_affairs_test_completed`, `mains_question_generated`, `mains_answer_submitted`, `mains_evaluation_completed`) in the backend activity manager and schemas, ensuring successful logging. Adjusted `UnifiedTestsService` and `TestsPage.tsx` to source prelims MCQ practice quizzes and Mains answer writing strictly from active, verified, fully extracted, and indexed `UPSCBook` study books instead of notes. Cleaned up all stale notes references and empty states, protected generation requests from duplicate submissions/React Strict Mode via refs, and verified that all 26 backend tests, startup smoke tests, and frontend build pipeline pass cleanly.
- **Completed Current Affairs Sourcing & Fallbacks:** Updated the backend and frontend to support fallback sourcing to other active publishers (like PIB, RBI, MEA, ForumIAS, InsightsIAS, Drishti IAS) when PWOnlyIAS articles are absent. Current Affairs views and quizzes use persisted accepted sources only; the temporary in-memory report dataset and its UI badges have been removed. Fixed the Q&A tab rendering defect to display grounded questions and answer/explanation previews properly instead of showing a hardcoded empty state.

- **Completed Automatic Current Affairs Ingestion from Official Government Sources (PIB, RBI, MEA):** Added centralized config (`config.py`) with official source URLs, allowed domains, timeout, max-byte, auto-ingest, and scheduler settings. Created `adapters.py` with `BaseSourceAdapter`, `PIBAdapter`, `RBIAdapter`, `MEAAdapter`, domain verification, and 5 MB response-size guards. PIB now falls back from its documented but empty RSS channel to the public English releases listing, retaining official `PRID` URLs and dates. Created `relevance.py` with deterministic UPSC relevance filtering, keyword subject/GS-paper classification, and extractive summaries. Created `ingestion_service.py` with an `asyncio.Lock`, DB stale-run cleanup, URL/content-hash deduplication, SQLite persistence (`is_demo=False`), deferred Chroma indexing, and accurate date/status metadata. Added the refresh/dates/status routes and ingestion CLI. Frontend API names and paths match the backend, and article responses expose `is_demo`. Live PIB verification fetched 50 releases, accepted and summarized 29, rejected 21, failed 0, and the second run reported 29 duplicates with no new cards. SQLite contains 39 non-demo official records (29 PIB, 10 RBI), all with source URLs and summaries. Targeted tests pass 37/37; the full backend suite passes 191 with 6 skipped and 1 warning; TypeScript and the frontend production build pass. Ollama was unavailable, so the 29 new PIB articles remain unindexed.


- **Completed UPSC Books classification/import correction:** Resolved two failing tests (`test_book_three_sections_classification_and_cli`, `test_detailed_duplicate_matching_and_classification`). Root causes: (1) `storage.py` startup ran a synthetic-book cleanup that deleted "Both Relevant Book" in CLI subprocesses (each new process executes `get_engine()` fresh, bypassing the `pytest in sys.modules` guard); (2) `canonical_url` was stored as `source_url#resource_id` — a non-unique fragment URL that broke Priority-4 duplicate detection and violated the `b.official_pdf_url == b.canonical_url` assertion; (3) `indexing_failures` counter incorrectly absorbed extraction failures; (4) `list_subjects` filtered by `content_status == "ready"`, causing subjects count to exclude dummy-PDF imports from tests. Fixes: removed startup synthetic-book cleanup entirely (replaced with explicit `cleanup_synthetic_books()` helper for admin use); changed `canonical_url` to stable internal identity URI `pwonlyias:book:{resource_id}`; separated extraction-failure counter (`not_ready`) from indexing-failure counter; removed `content_status` filter from `list_subjects`; made indexing assertions environment-agnostic (Ollama available → `indexed`, Ollama unavailable → `indexing_skipped`). All 9 `test_upsc_books.py` tests pass.
- **Completed UPSC Books Tab and Q/A Bank Feature:** Added three primary sections (Prelims Books, Mains Books, and Q/A Bank) to the UPSC Books interface. Updated `UPSCBook` schema to support `resource_kind` ('study_book' | 'qa_bank'). Added automatic database migrations, and mapped "Post Independence India Prahaar 2026" under Mains. Updated batch and single CLI PDF importers to accept and validate `--qa-bank` parameters. Modified books/subjects API query parameters to filter by active sections. Added premium three-tab navigation layout to `UpscBooksPage.tsx` with section-specific empty states, category badges, and dynamic practice/quiz buttons disabled or enabled based on book relevance and indexing status. Verified all 19 backend tests and completed clean production build compilation.
- **Completed Manual PDF Importer & Two-Tab Website Reader:** Added `backend/src/scripts/import_pwonlyias_pdf.py` CLI module to manually import official PWOnlyIAS PDFs, executing validation, checksum verification, storage organization, text extraction (`extract_pdf_blocks`), chapter detection for books, SQLite record merges, and ChromaDB/Ollama vector indexing. Created secure local PDF streaming endpoints `GET /upsc-notes/{note_id}/pdf` and `GET /upsc-books/{book_id}/pdf` with path traversal protections. Created a two-tab viewer in frontend reader modals ("Read Material" and "Original PDF") rendering original PDFs securely with Open PDF fallbacks. Added note/book card actions to view PDFs inline.
- **Completed Batch PDF Importer:** Created `backend/src/scripts/batch_import_pwonlyias_pdfs.py` CLI module to batch import Book PDFs from a folder matching metadata from a CSV file. It handles error isolation (continues batch when single files fail), rejects unmapped files unless overridden, validates official PWOnlyIAS urls, detects duplicates, supports `--dry-run`, and prints a detailed JSON summary report containing total, success, duplicate, image-only, extraction failure, indexing failure, and validation failure counts. Added automated tests verifying success and failure batch behaviors.
- **Completed Unified Tests Module**: Three-tab Tests page (`/tests`) replaces the Quizzes sidebar item. Backend: `backend/src/tests_engine/models.py` (MainsTestSession, MainsQuestion, MainsAnswerAttempt), `backend/src/tests_engine/service.py` (UnifiedTestsService with eligibility guards, UPSCNote.normalized_subject column references, half-mark rubric scoring), and `backend/src/api/routes/tests.py` (GET /tests/sources, POST /tests/prelims/generate, POST /tests/prelims/{id}/submit, POST /tests/mains/generate, POST /tests/mains/submit). Frontend: `TestsPage.tsx` with Prelims MCQ, Current Affairs Quiz, and Mains Answer Writing tabs; animated ScoreRing, RubricRow, word counter, question card, and result breakdown components. 14 backend Tests tests pass (6 skip when no real books/CA). 186 total backend tests pass, 6 skip, 12 pre-existing CA-quiz failures unchanged. Frontend production build passes.
- Completed UPSC Books Feature (PWOnlyIAS Static Books, Chapter Support & Web Reader): Purged synthetic test books (`Isolated Test Book`) from database & learner APIs. Added mandated **Select Subject** dropdown control (`MANDATED_SUBJECTS`), subject-aware query filtering (`?subject=...`), subject-specific empty states, detail-page link crawling in `discover_books`, and `import_from_official_source_page` service method. Storage models include `BookCollection`, `UPSCBook`, `BookChapter`, `SavedBook`, `BookReadingProgress`, reusing `is_valid_pwonlyias_source_url` and `extract_pdf_blocks` for Chroma RAG indexing (`source_type = "upsc_book"`). Responsive `UpscBooksPage.tsx` features high-contrast dark navy theme styling, honest empty state card with Check for Books action, chapter selector, and internal webpage reader modal with Table of Contents, page references, debounced reading-progress updates, and secondary official PWOnlyIAS PDF/source links.
- Completed UPSC Notes Feature (PWOnlyIAS Static Notes & Web Reader): Added mandatory **Select Subject** dropdown control (`MANDATED_SUBJECTS`) with live real counts (`subjectCounts`), subject-aware API query filtering (`?subject=...`), subject-specific empty states, and highlighted active subject overview cards. Cleaned synthetic test notes (`Prog Test`) from production DB & APIs, deduplicated Continue Reading notes, and updated `UpscNotesPage.tsx` with sleek high-contrast dark navy application theme styling (`#1e293b`/`#0f172a`, border `#334155`, text `#f8fafc`). Added strict official PWOnlyIAS URL validation (`is_valid_pwonlyias_source_url`), robust `pypdf` block extraction (`extract_pdf_blocks`), checksum deduplication, Chroma RAG indexing, and learner-facing API (`/upsc-notes/subjects`, `/collections`, `/`, `/{id}`, `/{id}/content`, `/{id}/save`, `/saved`, `/{id}/progress`).
- Completed Task A (PWOnlyIAS-Only Current Affairs): Purged synthetic test records (`Pending Backfill`, `Image Only PDF`, `Mode Test`, `Internal Reader Test`, `July Week 3`, `July 2026`) from database & APIs. Verified deduplication and 5 primary navigation sections (1. Day-wise, 2. Weekly-wise, 3. Monthly-wise, 4. Subject-wise, 5. Q&A). Added mandatory `Select Subject` dropdown control (`MANDATED_SUBJECTS`), dark theme styling (`#1e293b`/`#0f172a`), and honest empty state cards across all tabs.
...
- Thirty-three targeted backend tests across Current Affairs pass 100%. Backend startup smoke test passes. Frontend typecheck (`npx tsc --noEmit`) and frontend production build (`npm run build`) pass with zero errors.
...
Local production deployment was verified on 2026-07-24: the FastAPI backend returned HTTP 200 at `http://127.0.0.1:8000`, `/health` reported the database ready and Ollama reachable, and the built Vite frontend returned HTTP 200 at `http://127.0.0.1:5173`.
...
## Frontend repair milestone — 2026-08-05

- Current Affairs now waits for date metadata before its first article query, selects the backend's latest available date, ignores stale request responses, and distinguishes an empty date from filters that yield no matches. Live API verification found 11 active articles on `2026-08-04`.
- Visual Roadmaps now accept an explicit `general` source mode that bypasses document retrieval while preserving schema validation, SVG rendering, persistence, history, and export. Material-backed source modes keep their grounding requirement.
- AI Study Coach now uses a stable three-column desktop workspace with the composer aligned to the center column, responsive drawer behavior, normal message wrapping, and honest starter actions instead of fabricated conversation history.
- Verification: targeted backend tests passed (48); frontend TypeScript and production build passed. Full backend collection is currently blocked by the root-level live `test_ollama.py` because Ollama is not running on port 11434.

## Visual Learning Ollama availability repair — 2026-08-05

- Generation now uses the configured `OLLAMA_BASE_URL` and canonical `OLLAMA_GENERATION_MODEL`; embeddings continue to use `OLLAMA_EMBEDDING_MODEL` independently.
- `/health` exposes safe structured Ollama reachability plus generation and embedding model availability. Visual generation returns actionable structured errors rather than collapsing all failures into a generic model-unavailable message.
- General Visual Roadmaps require no RAG material. Detail level is now sent explicitly, model JSON accepts fenced/unfenced objects, unsafe markup and non-chronological timelines are rejected, and valid output continues through the real SVG/history pipeline.
- Local diagnosis: Ollama is not installed on the usual Windows paths and both `127.0.0.1:11434` and `localhost:11434` refuse connections. Install/start Ollama, then run `ollama pull qwen2.5:3b` if that configured model is absent.
- Verification: 30 affected tests passed; broader backend tests produced 205 passed, 6 skipped, and one unrelated existing demo-seed failure. Backend import, frontend typecheck/build, and `git diff --check` passed.
- Live completion: Ollama `0.32.1` is running, `qwen2.5:3b` is installed, and Visual Roadmaps use Ollama's native JSON response mode. A live General timeline completed as `ready`, rendered a 2,840-byte SVG, used the `General UPSC knowledge` label, and appeared in persisted visual history.

## User-provided Prelims book import — 2026-08-05

- Imported 11 unique user-provided PDF books (1,417 pages) into the real UPSC Books database and private local PDF storage. The duplicate `Art and Culture (1).pdf` was excluded by SHA-256 equality, and `Experiment 1 (1).docx` was intentionally ignored.
- All imported records are Prelims-only and classified into Art and Culture, History, Indian Polity and Governance, Indian Economy, Geography, Environment and Ecology, Science and Technology, and CSAT.
- Books APIs now return each record's real provider rather than hardcoding PWOnlyIAS, and listing/subject/collection queries include active user-provided books. Local imports use non-public source identifiers and never expose filesystem paths.
- Live verification found all 11 records through `/upsc-books?section=prelims`; ranged PDF streaming returned HTTP 206, `application/pdf`, and a valid `%PDF` signature.
- Text extraction is ready for every book. RAG indexing is currently skipped until the separately configured `nomic-embed-text` Ollama model is installed.

## User-provided PRAHAAR Mains book import — 2026-08-05

- Imported 15 unique PRAHAAR PDFs (1,642 pages) into the real Mains Books collection as user-provided, Mains-only resources. The separately listed Prelims geography PDF was excluded as requested.
- Subjects include Art and Culture, Environment and Ecology, Ethics, Geography and Disaster Management, Governance, History, Indian Economy, Indian Polity and Governance, Indian Society and Social Justice, Internal Security, International Relations, and Science and Technology.
- All PDFs have ready text extraction and private local storage. Live `/upsc-books?section=mains` verification returned all 15 records; ranged PDF streaming returned HTTP 206, `application/pdf`, and a valid `%PDF` signature.
- RAG indexing remains skipped until `nomic-embed-text` is installed; reading, searching metadata, filtering, saving, progress, and PDF access are available independently.

## Phase 3 learner workspace redesign — 2026-08-05

- Revision Center is now an action-focused real-data workspace backed by mastery and forgetting-risk APIs. It shows due work, explicit risk labels and reasons, estimated effort, mastery/risk/load analytics, and an active-recall dialog that records confidence-based mastery evidence before advancing.
- Progress Analytics now derives metrics, trends, distributions, and deterministic insights from activity, mastery, Current Affairs retention, and quiz-attempt APIs. Loading, partial failure, empty-data, retry, and responsive states are included; unsupported export and fabricated milestone/history data are intentionally omitted.
- Tests Center supports a General Subject Prelims source that does not require an indexed book, while preserving book-grounded and accepted Current Affairs paths. Partial submissions retain the total denominator, report answered/unanswered counts, preserve answers on failure, prevent duplicate clicks, and provide an accessible navigator plus mark/clear controls.
- User-provided extracted books are valid test sources even when semantic indexing was skipped; quiz generation reads their stored extracted blocks directly.
- Verification: frontend TypeScript and production build passed; focused backend activity/mastery/tests suite passed with 19 passed and 6 skipped. The full backend suite exceeded the 120-second validation window and was not repeatedly run. Browser screenshot QA was unavailable because no controllable browser instance was exposed in this session.

## Tests Center reference refinement — 2026-08-05

- The Tests Center now follows the approved builder hierarchy with real activity metrics, test type and source selection, free-form subject selection with UPSC suggestions, optional topic focus, difficulty, question count, and timed or untimed modes.
- General Subject generation accepts the selected topic and remains independent of indexed books. The active workspace shows one question at a time with a live countdown, answered progress, accessible navigator states, previous/next controls, marking, clearing, and partial submission.
- Real-data monthly calendars were added to Revision Center and Progress Analytics. Calendar markers come only from recorded revision dates or activity daily breakdowns; blank days remain explicitly empty.
